...
class MainPage extends React.Component {
    constructor(props){
        super(props);
        this.componentArr = []; // 对应的左侧菜单的内容组件
        this.contentArr = []; // 对应增删改查页面的组件
        this.state = {
            collapsed: false,
            mode: 'inline',
            userName: 'lxy',
            ...
            <Layout>
                ...
                <Layout>
                    <Sider breakpoint="lg" collapsible collapsed={this.state.collapsed} onCollapse={this.onCollapse} collapsedWidth="0" >
                        <Menu theme="dark" mode={this.state.mode} defaultSelectedKeys={['6']}></Menu>
                    </Sider>
                    <Layout style={{ background: '#f1f3f6' }}>
                        <Content style={{ padding: "10px 24px", margin: 0 }}>
                            <Redirect to="/main/modifyPwd" />
                            <Route path="/main/modifyPwd" component={componentRoute.modifyPwd} />
                        </Content>
